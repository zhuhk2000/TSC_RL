...
class SumoEnv(gym.Env):
    ...
    def create_simulation(self, random_seed: int = 0):
        t = str(time.time())
        if self.gui:
            sumo_cmd = [SUMO_GUI, '-c', self.sumocfg, "-W", "True", "--seed", str(random_seed)]
        else:
            sumo_cmd = [SUMO_BIN, '-c', self.sumocfg, "-W", "True", "--seed", str(random_seed)]
    ...
    def reset(self, initial_steps: int = 5, random_seed: int = 0):
        ...
        self.info: Dict[str:List] = defaultdict(list)
        return obs, None, None, self.info
